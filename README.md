...
应用默认使用“公开数据（腾讯+AKShare备用）”：先请求腾讯真实历史行情，失败后再尝试 AKShare。不同公开来源的缓存彼此隔离，避免缓存混淆真实来源。

没有外网时，可选择“CSV真实历史数据”上传自己的实际日线文件；至少需要 `日期、开盘、最高、最低、收盘` 列，可选 `成交量、成交额`。CSV内容必须来自用户自己的真实数据，应用不会生成补充价格。
