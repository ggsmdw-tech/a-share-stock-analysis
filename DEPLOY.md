...
当前版本：v0.02 数据可信度版。对外分享前，建议先由内部测试人员核对数据日期、数据可信度中心、市场对照、评分依据和模拟交易流程。
...
3. 创建应用时选择主分支，入口文件填写 `app.py`；在 Advanced settings 中选择 Python 3.12，然后点击部署。
...
- 公开网址仍按浏览器会话隔离，不能依赖部署主机上的 SQLite 保存多人长期数据；正式多人版本应增加登录和外部数据库。
